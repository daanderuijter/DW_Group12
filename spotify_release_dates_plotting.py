...
data_year = '2017'
df = pd.read_csv('output/dataframes/' + data_year + '/release_date.csv',index_col=0).loc[:,'1':'200']
...
ax1.set_title('Average Age Of Albums By Month In ' + data_year)
ax1.set_xlabel('Group of 4 weeks')
ax1.set_ylabel('Album Age (days)')
df2.transpose().mean().plot(ax=ax1, figsize=(12,4), legend=False)

ax2 = fig.add_subplot(122)
ax2.set_title('Average Age Of Tracks By Rank In ' + data_year)
ax2.set_xlabel('Track rank')
ax2.set_ylabel('Album Age (days)')
df3.plot(ax=ax2, figsize=(12,4), legend=False)

fig.savefig('output/figures/' + data_year + '/track_age.png')